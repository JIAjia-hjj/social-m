<template>
  <div class="writepage">
    ...
    <div class="footer fixed">
      ...
      <!-- 图片 -->
      <span class="iconfont" @click="addImg">
        <b class="imgnumber" v-show="contentimgurls.length>0">{{contentimgurls.length}}</b>&#xe833;
      </span>
      <!-- 视频 -->
    ...
    <div class="edit" ref="edit" v-show="isEdit">
      ...
      <div class="addcommunity" style="display:none">
        ...
        <div class="label ">选择/新增情绪标签 <span class="selectedlabel" >{{contentLabel}}</span>
          <div class="emotionlabelbox emotion_list clearfix">
            <ul>
              <li class="emotionlabel emotion_item " :class="{emotion_active:isEdit&&item==this.contentLabel}" v-for="item in emotionLabels" @click="selectLabel">{{item}}</li>
            </ul>
...
<script>
  import {submitWriteContent, updateWriteContent,getContentDetail} from 'network/api';
  ...
  export default {
    ...
    methods: {
      ...
      submitWriteContent() {
        ...
        if (this.$route.fullPath == "/write") {
          ...
          data = {
            ...
            'week': this.nowTime['week'],
            'isaddcommunity': this.isAddCommunity,
            'content': this.writeContent,
            'contentimgurls': this.contentimgurls.join(','),
            'contentlabel': this.contentLabel
          }
          submitWriteContent(data).then((res)=>{
            if(res.code!="000"){
              alert('服务器出错')
              return
            }
          ...
          updateWriteContent(data).then((res)=>{
            if(res.code!="000"){
              alert('服务器出错')
              return
            }
      ...
      showEditContent() {
        ...
        getContentDetail(data).then((res)=>{
          if(res.code!="000"){
            alert('服务器出错')
            return
          }
          this.writeContent = res.result[0]['content']
          if (res.result[0]['contentimgurls'].length > 0) {
            this.contentimgurls = res.result[0]['contentimgurls'].split(',')
          }
          document.getElementsByClassName("selectedlabel")[0].style.display = 'inline'
          this.contentLabel = res.result[0]['contentlabel']
...
<style scoped>
  ...
  .addcommunity  .emotion_active{
    background-color:#3597D4;
    color: #fff;
  }

  .addcommunity .label .emotion_list .addlabel {
    float: left;
    margin: 6px 4px;
    padding: 3px 8px;
  }
